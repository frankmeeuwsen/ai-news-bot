...
Transactional email service with better deliverability than Gmail SMTP
"""
import os
...
class ResendNotifier:
    ...
    def __init__(
        ...
        if not all([self.api_key, self.email_from, self.email_to]):
            logger.warning(
                ...
                "Required: RESEND_API_KEY, RESEND_FROM (or default), EMAIL_TO"
            )
        else:
            logger.info(f"ResendNotifier initialized (from: {self.email_from}, to: {self.email_to})")
    ...
    def send(self, content: str, subject: Optional[str] = None, language: str = "en") -> bool:
        ...
        Args:
            ...
            language: Language code to include in subject (e.g., 'en', 'nl', 'zh')
